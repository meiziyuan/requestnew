...
def interface(task_name,failcount,email_data):
    ...
    #新报告
    runner = HTMLTestRunner(stream=fp, title=u'测试', description=u'接口报告的描述')
    ...
    '''
    #把执行表新增一次执行任务
    CarryTask.objects.create(task_name=task_name,htmlreport=filename,successlogname=Successlogname,errorlogname=Errorlogname)
